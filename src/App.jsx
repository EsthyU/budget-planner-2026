...
export default function App(){
  ...
  const acts=useMemo(()=>{const a=[];debts.forEach(d=>{const pd=d.s.reduce((x,v,i)=>x+(d.p[i]?v:0),0);d.s.forEach((v,mi)=>{if(v>0)a.push({id:`${d.id}-${mi}`,task:`Pay ${d.n} \u2014 ${fmt(v,"RON",rates)}`,mo:MONTHS[mi],st:d.p[mi]?"Done":"Pending"})})});owed.forEach(o=>{if(o.st!=="Collected")a.push({id:`ow-${o.id}`,task:`Collect from ${o.n} \u2014 ${fmt(o.a,"RON",rates)}`,mo:"Ongoing",st:o.st==="Partial"?"In Progress":"Pending"})});return a},[debts,owed,rates]);

  const tabs=[{id:"dash",l:"Dashboard"},{id:"mo",l:"Monthly"},{id:"inc",l:"Income"},{id:"debt",l:"Debts"},{id:"save",l:"Savings"},{id:"owe",l:"Owed to Me"},{id:"plan",l:"Plans"},{id:"act",l:"Actions"},{id:"set",l:"Settings"}];
...
{/* ACTIONS */}
{tab==="act"&&(<div style={{display:"flex",flexDirection:"column",gap:16}}>
<Panel><Title sub="Auto-generated from debts & collections">Action Plan</Title>
<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:16}}>
<Stat label="Total" value={acts.length} color={C.gd}/>
<Stat label="Done" value={acts.filter(a=>a.st==="Done").length} color={C.gn}/>
<Stat label="Pending" value={acts.filter(a=>a.st!=="Done").length} color={C.rd}/></div>
<Bar value={acts.filter(a=>a.st==="Done").length} max={acts.length} color={C.gn} h={8}/>
<div style={{marginTop:16}}>{acts.map(a=>(<Row key={a.id} style={{opacity:a.st==="Done"?.4:1}}>
<div style={{width:8,height:8,borderRadius:"50%",background:a.st==="Done"?C.gn:a.st==="In Progress"?C.gd:C.rd,flexShrink:0}}/>
<div style={{flex:1,marginLeft:4}}>
<div style={{fontSize:13,fontWeight:600,textDecoration:a.st==="Done"?"line-through":"none"}}>{a.task}</div>
<div style={{display:"flex",gap:4,marginTop:4}}><Badge color={C.bl}>{a.mo}</Badge><Badge color={a.st==="Done"?C.gn:C.mt}>{a.st}</Badge></div></div></Row>))}</div>
{acts.length===0&&<div style={{textAlign:"center",padding:32,color:C.mt}}>No active debts or collections. You're on track!</div>}
</Panel></div>)}
